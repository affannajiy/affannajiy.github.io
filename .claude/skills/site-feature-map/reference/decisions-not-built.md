...
Recorded so nobody re-proposes them as improvements. A suggestion here is not "we
forgot". It is "we decided, and here is why".
...
| Thing | Why not |
| --- | --- |
| **Hover-prefetch of the certificate PDFs** (the mcmaster.com behaviour) | `default-src 'none'` blocks `<link rel="prefetch">` for this origin's own files, and Chrome dropped `prefetch-src`, so it falls back to `default-src`. Buying it means `default-src 'self'`, which downgrades the CSP from "nothing loads unless named" to "anything same-origin loads", to speed up seven PDFs behind seven links. McMaster prefetches because it has thousands of pages behind a hover. **Not a trade worth making here.** |
...
| **A permanent "claimed by" column on Education, Experience and Certificates** | The reverse of the evidence links, always visible. A fourth column costs width at 375px and lines in the one-page résumé. The transient caption in `.jump-note` answers the same question at the moment it is asked, then disappears. |
...
| **Fuzzy search, `aria-activedescendant` combobox rewrite** | Not wrong, just not worth it yet. The parser and arrow-key walking already work, and a rewrite risks the keyboard path for a small gain. Revisit if results ever outgrow one screen. |
...
All 213 items of the OWASP Secure Coding Practices checklist walked. That list
was §2a of [SECURITY_Rulebook.md](../../../../rulebooks/SECURITY_Rulebook.md) until 2026-08-20 and is
now replaced there by ASVS 5.0. Four changes shipped ([security-posture.md](../../audit-untrusted-input/reference/security-posture.md) §1, §2, §4, §7). These
...
| **`redirect: "error"` on the GitHub fetch** (SCP-10) | GitHub answers `301` for a renamed account, so this breaks the table the day Affan renames himself. It is a self-inflicted outage guarding a path `connect-src https://api.github.com` already closes, because the CSP is checked on every redirect hop. |
| **`robots.txt` disallowing `rulebooks/` and `.claude/`** (SCP-158) | Nothing to hide. The repo is public, so the crawler's copy discloses nothing new. It costs a fifth top-level file, which is how rule 1.4 erodes. |

Not rejected, just **not ours**: response headers (SCP-162), HSTS (SCP-143) and
...
| **Un-scrolling the condensed nav strip** | It scrolls sideways deliberately, with a `mask-image` fade that says so ([layout.md](../../site-design-and-layout/reference/layout.md) §3). It is a control strip, not content, and the alternative is dropping destinations. |
...
## Rejected from the 2026-08-20 UI/UX rulebook round

| Thing | Why not |
| --- | --- |
| **A 65ch measure on body prose** (UI-UX §6.10) | Built, measured, and reverted the same day. Line length at 1280px was 108ch for bare prose and 144ch for `.hint`, which is over the 80ch ceiling. But the paragraphs on this page are short, so the cap crunched them into a narrow strip beside a wide empty gutter, and that reads worse than the long line does. The 1000px column stays the only bound. Two facts survive the revert for anyone who tries again — see [layout.md](../../site-design-and-layout/reference/layout.md) §1. |
...
| Mirror on Cloudflare Pages / Netlify now | Real redundancy, but it means a standing second host, a second account and a DNS decision. That is furniture, for a rare event. Documented as the option if it is ever wanted. Cloudflare, because it serves static files with no build and no injected analytics. |

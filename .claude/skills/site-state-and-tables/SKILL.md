...
- **Never build HTML from remote data.** Build nodes, set `textContent`. There is
  deliberately no `escapeHTML()`. Trusted Types makes any `innerHTML` write
  throw — clear with `textContent = ""`.
...
- **A stated failure needs a way to act on it.** Every error path names the fix
  and offers Retry.
- **Exercise controls, do not read them.** Sort, filter and Retry each looked
  correct in the markup while broken in fact.

Run `verify-site` after any change here. Run `audit-untrusted-input` too if the
